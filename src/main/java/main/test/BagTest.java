...
public class BagTest {
	...
	public void main(String[] args) {
		...
		Transaction transaction = new Transaction("Test Transaction", "Test", test1.getID(), LocalDate.now(), new BigDecimal("125.12345"));
		addTransaction(ledger, transaction);
		displayAccount(test0);
		displayAccount(test1);
		System.out.println("\n");

		Transaction transaction1 = new Transaction("Test Transaction 1", "Test Description", "Test", test1.getID(), LocalDate.now(), new BigDecimal("-20.023"));
		addTransaction(ledger, transaction1);
		displayBalance(ledger);

		displayTransactions(ledger);

		remTransaction(ledger, transaction);	
		displayTransactions(ledger);
	}
	...
	public static void remTransaction(Ledger ledger, Transaction transaction) {
		try {
